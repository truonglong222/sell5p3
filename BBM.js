...
async function main() {
 ...
 // BƯỚC 2: Lấy nến 4H và 1H đồng thời
 ...
 // Tính diffema10 trên khung 4H (bỏ nến [0] đang chạy)
 const closedCandles4h = candles4h.slice(1);
 const closes4hClosed = closedCandles4h.map(c => parseFloat(c[4])).reverse();
 const ema4hArray = calculateEMAArray(closes4hClosed, 20);

 // Cần tối thiểu 10 giá trị EMA để so sánh nến 1 và nến 10
 if (ema4hArray.length < 10) {
 await sleep(80);
 continue;
 }

 // ema1: EMA20 tại nến 4H vừa đóng (index 1), ema10: EMA20 tại nến 4H số 10 (cách 9 nến trước đó)
 const ema1 = ema4hArray[ema4hArray.length - 1];
 const ema10 = ema4hArray[ema4hArray.length - 10];
 const diffema10 = ema10 > 0 ? ((ema1 - ema10) / ema10) * 100 : 0;

 // BƯỚC 3: Tính Bollinger Bands khung 1H (20 nến gần nhất từ nến vừa đóng)
 ...
 // BƯỚC 4: Kiểm tra điều kiện Long / Short theo diffema10
 // Long: 1% < diffema10 < 3% VÀ -3% < bbd1h < -0.5%
 const isLong = diffema10 > 1 && diffema10 < 3 && bbd1h > -3 && bbd1h < -0.5;

 // Short: -3% < diffema10 < -1% VÀ 0.5% < bbt1h < 3%
 const isShort = diffema10 > -3 && diffema10 < -1 && bbt1h > 0.5 && bbt1h < 3;
